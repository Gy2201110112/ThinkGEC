...
def compute_score(solution_str: str,
    ...
    if format_correct and answer_text and len(answer_text) < 20:
        predict = re.findall(r"[a-zA-Z]", answer_text)
        print(f"  Ground Truth: {ground_truth}")
        print(f"  Answer: {predict}")
        p, r, f = compute_prf(predict, ground_truth)
        print(f"\n[Content Validation]")
        if p == 1:
