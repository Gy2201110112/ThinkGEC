...
def batch_multi_pre_rec_f1(candidates, sources, gold_edits, max_unchanged_words=2, beta=0.5, ignore_whitespace_casing= False, verbose=False, very_verbose=False):
    min_sentence_length = min(len(candidates), len(sources))
    candidates = candidates[:min_sentence_length]
    sources = sources[:min_sentence_length]
    gold_edits = gold_edits[:min_sentence_length]
